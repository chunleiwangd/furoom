...
public class HttpParse {
	...
	public HttpParse(HttpServletRequest request, HttpServletResponse response)
		...
		pathInfo=pathInfo.substring(FR_PATH_PATTERN.length());
		...
		if (queryString != null
			...
			String ptstr = null;
			if (ctxType == null || !ctxType.startsWith("x-application/fr:")) {
				ptstr = request.getParameter(FR_PROTOCOL_FULLNAME);
				if (ptstr != null && ptstr.length() > 0) {
					if (ptstr.startsWith("fr:")) {
						ptstr = ptstr.substring(3); // "remove fr:"
					}
				} else { // search in attribute
					ptstr = (String) request.getAttribute(FR_PROTOCOL_FULLNAME);
					if (ptstr != null && ptstr.length() > 0) {
						ptstr = ptstr.substring(3); // "remove fr:"
					}
				}
			} else {
				ptstr = ctxType.substring("x-application/fr:".length());
