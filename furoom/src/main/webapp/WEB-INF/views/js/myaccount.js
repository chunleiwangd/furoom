...
var inviteview = function(container){
	var inviteState={1: '未注册', 2 : '正在注册', 4: '已注册', 8: '已购买'};
	var datas = null;
	...
	notice.append(inviteMessage);
	notice.append("<button class='apply' style='float:right;'>申请新邀请码</button>");
	notice.append("<div style='clear:both;'></div>");
	
	/*var seldiv = $('<div class="col-md-12" style="padding-left:0px; padding-right:0px;"></div>');
	seldiv.append('<span style="font-size:18px; color:#999" id="category">全部</span>');
	
	var sel = $('<select id="state" style="float:right;"></select>');
	sel.append('<option value="-1">全部</option>');
	sel.append('<option value="1">未注册</option>');
	sel.append('<option value="4">已注册</option>');
	sel.append('<option value="8">已购买</option>');
	seldiv.append(sel);
	
	seldiv.append('<div style="clear:both;"></div>');
	*/
	...
	$('button.apply').click(function(){
		try{
			inviteService.allocateByLender(5);
			alert("分配成功");
			window.location.href="myaccountdetail.html?fid=mycenter&sid=invite-view";
		}catch(e){
			alert(e.message);
		}
	});
		...
		if(items)
		{
			for(var i=0; i<items.size(); i++){
				...
				if(item.handleFlag==2)
				{
					...
					pbbutton = "<button disabled='disabled'>持有未满15天</button>";
				}
				result.aaData.push(["<a href='productdetail.html?pid="+item.product.id+"' >"+item.product.govermentOrder.title+"("+item.product.productSeries.title+")</a>",
...
var paybackall = function(container){
	var repayedDetail=accountService.getLenderRepayedDetail();
	var willBeRepayedDetail =accountService.getLenderWillBeRepayedDetail();
...
//		res = paybackService.findBorrowerPayBacks(2, -1, -1, iDisplayStart, iDisplayLength);
		res = accountService.findLenderRepayCashStream(iDisplayStart, iDisplayLength);
		var result = {};
		result.iTotalRecords = res.get('total');
		result.iTotalDisplayRecords = res.get('total');
		result.aaData = new Array();
...
var paybackto = function(container){
	var paybacks = accountService.findLenderWaitforRepay();
...
var cashProcessor=function(action,state,container){
	var fnServerData = function(sSource, aoData, fnCallback, oSettings) {
		...
		var res = null;
		res = accountService.findLenderCashStreamByActionAndState(action,state, iDisplayStart, iDisplayLength);
		var result = {};
		result.iTotalRecords = res.get('total');
		result.iTotalDisplayRecords = res.get('total');
		result.aaData = new Array();
...
var letterreaded = function(container){
	...
	var fnServerData = function(sSource, aoData, fnCallback, oSettings) {
		...
		var res = null;
		res = letterService.findMyLetters(1, iDisplayStart, iDisplayLength);
		var result = {};
		result.iTotalRecords = res.get('total');
		result.iTotalDisplayRecords = res.get('total');
		result.aaData = new Array();
		...
		$('.readletter').click(function(e){
			...
			$('#rmyModalLabel').html(letter.title);
			$('#rldetail').html(letter.content);
